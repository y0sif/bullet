/// Phase A variant C — Pure B-spline KAN (no base path).
///
/// Output is entirely `spline_weight @ bspline_basis(input)`.
/// Tests whether the learned spline activations alone are sufficient.
use bullet_lib::{
    game::{
        formats::sfbinpack::{
            TrainingDataEntry,
...
fn main() {
    let mut trainer = ValueTrainerBuilder::default()
        ...
        .build(|builder, stm_inputs, ntm_inputs| {
            ...
            let kan1 = kan_layer(builder, "kan1", 2 * FT_SIZE, KAN_HIDDEN, GRID_SIZE, SPLINE_ORDER, (-1.0, 1.0), false);
            let hidden = kan1.forward_pure(ft_out);

            let hidden_clamped = hidden.clip_pass_through_grad(-1.0, 1.0);

            let kan2 = kan_layer(builder, "kan2", KAN_HIDDEN, 1, GRID_SIZE, SPLINE_ORDER, (-1.0, 1.0), false);
            kan2.forward_pure(hidden_clamped)
        });

    let schedule = TrainingSchedule {
        net_id: "kan-variant-c".to_string(),
