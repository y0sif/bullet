/// Phase A variant A — Control: B-spline KAN with SiLU base.
///
/// Architecture: 768 -> ft(128) CReLU -> KAN(256->128) -> KAN(128->1)
/// Base activation: SiLU (`x * sigmoid(x)`), matches kan_simple.rs.
///
/// Differs from kan_simple.rs only in net_id so checkpoints don't collide.
use bullet_lib::{
    game::{
        formats::sfbinpack::{
            TrainingDataEntry,
...
fn main() {
    ...
    let schedule = TrainingSchedule {
        net_id: "kan-variant-a".to_string(),
